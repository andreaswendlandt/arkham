...
check_cronguard() {
    if [ -z "$sudo_pid" ]; then
        if ! [ -z "$oldpid" ] && ps -ef | grep "$daemon" | egrep -v "grep|$pid|mysql" > /dev/null 2>&1; then
            # Daemon is running
            return 1
        else
        ...
        if ! [ -z "$oldpid" ] && ps -ef | grep "$daemon" | egrep -v "grep|$pid|$sudo_pid|mysql" > /dev/null 2>&1; then
            # Daemon is running
            return 1
	else
            # Daemon isn't running.
...
# Cronguard Function
cronguard() {
    query=$(mysql -u $db_user -p$db_password $db -e"SELECT token,result,host,ident FROM $table" 2>&1 | grep -v 'Using a password' | tail -n +2)
    if ! [ -z "$query" ]; then
        echo -e "$query" | while read line; do
            set $line
	    if [ "$2" == "success" ]; then
                log "$4 will be removed"
	        if mysql -u $db_user -p$db_password $db -e"DELETE FROM $table WHERE ident = \"$4\"" 2>&1 | grep -v 'Using a password'; then
                    log "$4 from $3 removed"
                fi
            elif [ "$2" == "fail" ]; then
                log "$4 will be sent"
	        ...
	        if ! grep "$4" $logfile >/dev/null 2>&1; then
                    log "$4 is still running"
		fi
	        current_time=$(date +%s)
