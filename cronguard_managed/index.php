  ...
  <body> 
    ...
    <div class="ui container">
      ...
      <div class="ui segment">
        ...
        It will send the following data(token, ident, host, start time, command and action) via curl to the cronguard server which writes them to a database, executes the cronjob and checks if the command(s)/script were successfully executed and makes then a second curl to the server with the result(failed or success). On the server runs a daemon which checks once per minute for new database entries, entries with a 'success' as a result will just be deleted, entries with a 'fail' as a result will be send per mail - and then deleted, if there is no result(NULL) and the cronjob is running longer than one day(86400 seconds) a mail will be send as well and the entry will be deleted. To get it working generate a token, store the token in /opt/cronguard/token.inc.sh (this is the location where the wrapper expects it), download the wrapper script cron_wrapper.sh and you can start. Beside generating a token you can - if you are not sure if your token is valid - validate your token and of course remove your token(and your email address). If you lost or forgot your token you can on the dedicated site let your token sent to you. This application has also a simple api, you can check if there are any entries(for your token), you can call this api like this:<br />
      ...
      <div class="ui segment">
        Only 2 things, the script cron_wrapper.sh which executes your cronjobs and a valid token. You can get both her, the cron_wrapper.sh script from the download site, and the token you can generate at the generate token site(all you need for that is a valid email address)
      </div>
